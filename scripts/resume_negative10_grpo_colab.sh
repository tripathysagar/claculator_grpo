...
SESSION="${COLAB_SESSION:-calc-negative10-strict5-a100-r3-20260724}"
RUN_TAG="${RUN_TAG:-a100negative10strict5-20260724}"
SFT_REPO="tripathysagar/qwen3-0.6b-calc-sft200-negative10-a100-20260724"
GRPO_REPO="tripathysagar/qwen3-0.6b-calc-grpo600-negative10-strict5-a100-20260724"
SFT_MANIFEST="outputs/evaluations/qwen3-calc-negative10-strict5-a100-20260724/sft_hub_revision.json"
SFT_REVISION="$(python -c 'import json,sys; print(json.load(open(sys.argv[1]))["revision"])' "$SFT_MANIFEST")"
...
TMP_NOTEBOOK_ROOT="/tmp/qwen3-calc-negative10-strict5-a100-resume-20260724"
...
colab upload -s "$SESSION" "data/calculator_qwen3_negative10_manifest.json" "/content/data/calculator_qwen3_negative10_manifest.json"
...
printf '%s\n' "$python_code" |
  colab exec -s "$SESSION" --timeout 1800 | tee "$LOCAL_ROOT/grpo-resume-install.log"
colab restart-kernel -s "$SESSION"
inject_secrets
set_grpo_controls
colab exec -s "$SESSION" \
  -f "$TMP_NOTEBOOK_ROOT/grpo_qwen3_calculator.ipynb" \
  --timeout 7200 | tee "$LOCAL_ROOT/grpo-resume-execution.log"
...
printf '%s\n' "exec(open('/content/upload_grpo_adapter.py').read())" |
  colab exec -s "$SESSION" --timeout 900 | tee "$LOCAL_ROOT/grpo-resume-upload.log"
...
colab log -s "$SESSION" -o "$LOCAL_ROOT/colab-session-resume.ipynb"
colab ls -s "$SESSION" "/content/experiments"
colab stop -s "$SESSION"
trap - EXIT
colab sessions
echo "NEGATIVE10_GRPO_RESUME_COMPLETE $LOCAL_ROOT"
